 #CHAPTER 5 - Going Further - 3
 #
 #      Make the program able to either operate on command-line arguments or
 #      use STDIN or STDOUT based on the number of command-line arguments
 #      specified by ARGC.
 #
 #PURPOSE: This program converts text from lower-case to upper-case.
 #         It operates on either STDIN or a file and writes to either
 #         STDOUT or a file.
 #
 #PROCESSING: 1) Determine input and output
 #            2) If input is a file then open the input file
 #            3) If output is a file then open the output file
 #            4) While we're not at the end of the input
 ...
 #
 #NOTE:    One argument will be considered input.
